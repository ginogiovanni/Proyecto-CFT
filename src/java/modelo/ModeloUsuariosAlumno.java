 ...
 * @author Hawk
 */
public class ModeloUsuariosAlumno extends Conexion{
    ...
    public ArrayList<UsuariosPersonal> getUsuariosPersonal() {
        ...
        try {
            String sql = "Call selectUsuariosAlumno()";
     ...
     public boolean crearUsuarioAlumno(UsuariosPersonal up) {
        boolean flag = false;
        PreparedStatement pst = null;
        try {
            String sql = "call insertUsuariosAlumno(?,?,?,?);";
     ...
     public boolean borrarUsuarioAlumno(int id){
        ...
        try {
            String sql = "call eliminarUsuarioAlumno(?)";
            pst = getConnection().prepareCall(sql);
            pst.setInt(1, id);
            if (pst.executeUpdate() == 1) {
                flag = true;
